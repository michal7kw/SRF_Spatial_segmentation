...
def run(images: ImageSet, properties: CellposeSegProperties, parameters: CellposeSegParameters) -> np.ndarray:
    ...
    mask = extract_masks_with_cellpose(properties, parameters, image, to_segment_z, channels, properties.use_gpu)
...
def extract_masks_with_cellpose(
    ...
    if properties.custom_weights:
        model = models.CellposeModel(gpu=use_gpu, pretrained_model=properties.custom_weights, net_avg=False)
    else:
        model = models.CellposeModel(gpu=use_gpu, model_type=properties.model, net_avg=False)
